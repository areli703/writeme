...
import { loadColumns } from './configureGenerated_textsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListGenerated_texts from './ListGenerated_texts';

const perPage = 10;

const TableSampleGenerated_texts = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: generated_textsNotify,
    refetch,
  } = useAppSelector((state) => state.generated_texts);
  ...
  useEffect(() => {
    if (generated_textsNotify.showNotification) {
      notify(
        generated_textsNotify.typeNotification,
        generated_textsNotify.textNotification,
      );
    }
  }, [generated_textsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/generated_texts/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/generated_texts/generated_texts-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {generated_texts && Array.isArray(generated_texts) && !showGrid && (
        <ListGenerated_texts
...
export default TableSampleGenerated_texts;
