...
import { loadColumns } from './configureGenerated_imagesCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListGenerated_images from './ListGenerated_images';

const perPage = 10;

const TableSampleGenerated_images = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: generated_imagesNotify,
    refetch,
  } = useAppSelector((state) => state.generated_images);
  ...
  useEffect(() => {
    if (generated_imagesNotify.showNotification) {
      notify(
        generated_imagesNotify.typeNotification,
        generated_imagesNotify.textNotification,
      );
    }
  }, [generated_imagesNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/generated_images/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/generated_images/generated_images-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {generated_images && Array.isArray(generated_images) && !showGrid && (
        <ListGenerated_images
...
export default TableSampleGenerated_images;
